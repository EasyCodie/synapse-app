...
export default function SignupPage() {
  ...
  if (emailSent) {
    return (
      <div className="bg-surface-1 border border-hairline rounded-lg p-6 space-y-3 text-center">
        <div className="w-10 h-10 rounded-full bg-semantic-success/10 flex items-center justify-center mx-auto">
          <svg className="w-5 h-5 text-semantic-success" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
          </svg>
        </div>
        <h2 className="text-card-title text-ink">Check your email</h2>
        <p className="text-body-sm text-ink-subtle">
          We sent a confirmation link to <span className="text-ink">{email}</span>.
          Click it to activate your account.
        </p>
        <Link href="/login" className="text-body-sm text-primary hover:text-primary-hover transition-colors">
          Back to sign in
        </Link>
      </div>
    );
  ...
  return (
    <div className="bg-surface-1 border border-hairline rounded-lg p-6 space-y-5">
      <div className="space-y-1">
        <h1 className="text-card-title text-ink">Create your workspace</h1>
        <p className="text-body-sm text-ink-subtle">
          Your IB academic OS starts here.
        </p>
      </div>
      ...
      <form onSubmit={handleSignup} className="space-y-3">
        <div className="space-y-1.5">
          <label htmlFor="name" className="text-body-sm text-ink-muted">
            Full name
          </label>
          <Input
            id="name"
