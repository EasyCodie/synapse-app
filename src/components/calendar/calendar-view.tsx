        ...
        <div className="lg:col-span-2 bg-surface-1 border border-hairline rounded-lg p-6">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-1">
              <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={prevMonth} className="flex items-center justify-center w-9 h-9 rounded-md hover:bg-surface-2 text-ink-subtle transition-colors duration-200">
                <ChevronLeft className="w-4 h-4" />
              </motion.button>
              <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={nextMonth} className="flex items-center justify-center w-9 h-9 rounded-md hover:bg-surface-2 text-ink-subtle transition-colors duration-200">
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "h-11 flex flex-col items-center justify-center rounded-md relative transition-all duration-200",
                      isToday && "bg-primary/10",
                      isSelected && "ring-1 ring-primary bg-primary/5",
                      !isToday && !isSelected && "hover:bg-surface-2/60"
                    )}
                  >
                    <span className={cn(
                      "text-caption",
          ...
          <AnimatePresence>
            {overdue.length > 0 && (
              <motion.div
                ...
                className="bg-surface-1 border border-red-500/20 rounded-lg p-6"
              >
    ...
    <div className="group flex items-center gap-2 py-1">
      <button
        ...
        className={cn(
          ...
          task.completed ? "bg-primary/10" : "hover:bg-surface-2"
        )}
      >
        <motion.div
          className={cn(
            "w-4 h-4 rounded border flex items-center justify-center transition-colors duration-200",
            task.completed ? "bg-primary border-primary" : "border-hairline-strong hover:border-primary"
          )}
      ...
      <div className="flex-1 min-w-0">
        <p className={cn(
          "text-body-sm truncate transition-all duration-200",
      ...
      {task.due_date && (
        <span className="text-caption text-ink-tertiary shrink-0 tabular-nums">
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
        <motion.div
          ...
          transition={{ duration: 0.2 }}
          className="pointer-events-auto bg-surface-1 border border-hairline rounded-lg p-6 w-full max-w-md mx-4"
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-card-title text-ink">New Task</h3>
            <button onClick={onClose} className="p-1.5 rounded-md hover:bg-surface-2 text-ink-subtle transition-colors duration-200">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2.5 rounded-md bg-surface-2 border border-hairline text-body-sm text-ink placeholder:text-ink-tertiary focus:border-hairline-strong focus:outline-2 focus:outline-primary/50 min-h-[44px]"
                autoFocus
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2.5 rounded-md bg-surface-2 border border-hairline text-body-sm text-ink focus:border-hairline-strong focus:outline-2 focus:outline-primary/50 min-h-[44px]"
                />
              </div>
            </div>
              ...
              <div className="flex gap-2">
                {(["low", "medium", "high", "urgent"] as const).map((p) => (
                  <button
                    ...
                    className={cn(
                      "flex items-center gap-1.5 px-3 py-2 rounded-md text-caption border transition-all duration-200 min-h-[36px]",
                      priority === p
